...
for sn in sns:
	if (sn,'hqs') not in memory and (not skip_calculation or load_hqs):
		...
		vecs = dat['vecs']
		mesh = dat['mesh']
		protein_lipid_contacts = data['protein_lipid_contacts'][sn]['data']
		contact_frames = protein_lipid_contacts['frames']
		...
		memory[(sn,'hqs')] = hqs
		memory[(sn,'vecs')] = vecs
		###memory[(sn,'hqs')] = hqs[np.where(contact_frames)]
		###memory[(sn,'vecs')] = vecs[np.where(contact_frames)]
